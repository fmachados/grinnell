...
Currently, input and output processing are done in R, while the simulation runs in Python. We are testing new functions to migrate to R and submit the package to CRAN.
...
Mac users might need to install packages like **rgeos** and **rgdal** manually or update XCode Command Line Tools.
...
knitr::include_graphics("README_files/occplot.png")
```
...
The output folder ("eg_msim1") includes a PCA directory, a suitability directory with a fundamental niche ellipsoid, and the accessible area (M) in shape format, among other outputs. You can check the report text file to see the simulation time and parameters used. A png image will show how the accessible area (blue outline) will look over the current suitability layer:
